...
static QString translateOptions(const QString &text)
{
	...
	for (int pos = 0, oldPos = 0; pos >= 0;)
	{
        ...
        translatedText += QStringView{text}.mid(oldPos, pos - oldPos + 1); // add text between the current option and the previous option; this also adds the end of the original string, except when there are no options
#else
